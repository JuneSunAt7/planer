...
class ExampleApp(QtWidgets.QMainWindow, design.Ui_MainWindow):
    ...
    def save(self):
        ...
        filemanagment.wr_to_main_file(str(self.lineInputTask.text()), str(times), str(self.difficultSlider.value()),
                                      str(self.resoursesList.toPlainText()).replace("\n", '&'))
        self.__clear()
        self.tableWidget.setVisible(True)
    ...
    def show_key(self):
        pixmap = QtGui.QPixmap('key.png')

        self.label.setPixmap(pixmap)
        self.label.resize(pixmap.width(), pixmap.height())
        self.label.setGeometry(317, 240, pixmap.width(), pixmap.height())
        self.label.show()
...
def main():
    ...
    app.setWindowIcon(QtGui.QIcon('auto.ico'))
    window = ExampleApp()
    window.setWindowIcon(QtGui.QIcon('auto.ico'))
    window.show()
